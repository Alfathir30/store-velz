...
const TELEGRAM_BOT_TOKEN = process.env.TELEGRAM_BOT_TOKEN || "8111172635:AAG6nTMKB-Mj4aZDEHn4ldC6sjB-FH8-zQ"
const TELEGRAM_CHAT_ID = process.env.TELEGRAM_CHAT_ID || "193786408"
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    const deliveryContact = params.get("delivery_contact") || ""
...
async function sendTelegramNotification(transactionId, orderData) {
  try {
    ...
    // APK Premium info
    if (orderData.apk_email && orderData.apk_password) {
      additionalInfo += `
📧 *Data Akun Premium:*
• Email: ${orderData.apk_email}
• Password: ${orderData.apk_password}
`
    }

    // Delivery info for APK Hack
    if (orderData.delivery_method && orderData.delivery_contact) {
      additionalInfo += `
📤 *Pengiriman Data:*
• Metode: ${orderData.delivery_method}
• Kontak: ${orderData.delivery_contact}
`
    }
